...
class AjaxRequest {
	...
	public function __construct($action, $post = array()) {
		...
		switch ($this->action) {
			...
			case "getListDatabases":
				Core::init();
				$dbHostname = Core::getHostname();
				$dbUsername = Core::getDbUsername();
				$dbPassword = Core::getDbPassword();
				$dbName     = Core::getDbName();
				$schema = new Schema($dbHostname, $dbUsername, $dbPassword, $dbName);
				$databases = $schema->getListDatabases();
				$this->response["content"] = $databases;
				break;
			...
			case "getListTables":
				...
				$dbName     = Core::getDbName();
				$schema = new Schema($dbHostname, $dbUsername, $dbPassword, $dbName);
				$tables = $schema->getListTables($this->post['dbName']);
				$this->response["content"] = $tables;
				break;
			...
			case "getListColumns":
				...
				$dbName     = Core::getDbName();
				$schema = new Schema($dbHostname, $dbUsername, $dbPassword, $dbName);
				$columns = $schema->getListColumns($this->post['dbName'], $this->post["table"]);
				$this->response["content"] = $columns;
				break;
