...
define([
	...
	var _run = function() {
		...
		$("#gdListDatabases").on("change", _onChangeLoadTables);
		$("#gdListTables").on("change", _onChangeLoadFields);
	...
	var _initListDatabases = function(){
		$.ajax({
			url: "ajax.php",
			type: "POST",
			dataType: "json",
			data: {
				action: "getListDatabases",
			},
			success: function(result) {
				$.each(result.content, function(index, database){
					$("#gdListDatabases").append("<option value='" + database + "'>" + database + "</option>");
				})
				_onChangeLoadTables();
			},
			error: function(json) {
	...
	var _onChangeLoadTables = function(e){
		var dbName = $("#gdListDatabases").val();
		$.ajax({
			url: "ajax.php",
			type: "POST",
			dataType: "json",
			...
			success: function(result) {
				$("#gdListTables").html("");
				$.each(result.content, function(index, table){
					$("#gdListTables").append("<option value='" + table + "'>" + table + "</option>");
				})
				_onChangeLoadFields();
			},
			error: function(json) {
	...
	var _onChangeLoadFields = function(e){ 
		var tableName = $("#gdListTables").val();
		var dbName = $("#gdListDatabases").val();
		$.ajax({
			url: "ajax.php",
			type: "POST",
			dataType: "json",
			data: {
				action: "getListColumns",
				dbName: dbName,
				table: tableName
			},
			success: function(result) {
				var rows = result.content.length;
				var rowIDs = [];
				var tableRowHTML = "";

				for(var currRow = 1; currRow <= rows; currRow++){ console.log(currRow);
					rowIDs.push(currRow);
					var newRowHTML = $("#gdTableRowTemplate").html().replace(/%ROW%/g, currRow);
					tableRowHTML += '<li class="gdTableRow" id="row_' + currRow + '">' + newRowHTML + '</li>';
				}

				$("#gdTableRows").html(tableRowHTML);
				...
				// push field name and data type
				$.each(result.content, function(row, column){
					console.log(column.name + ' - ' + column.data_type.name + column.data_type.options) ; 
					var currRow = ++row;
					$("#gdTitle_" + currRow).val(column.name);
					$("#gdDataType_" + currRow).val("data-type-" + column.data_type.name);
					_publishDataTypeChange($("#gdDataType_" + currRow)[0]);
					if (column.data_type.options.length){
						$.each(column.data_type.options, function(index, option){
							$("#dt" + option.name + "_" + currRow).val(option.value);
						})
					}
